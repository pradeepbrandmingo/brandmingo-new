import React from "react";

const CRMWhyChooseBm = () => {
  return (
    <section className="wcb-section">
      <div className="auto-container">
        {/* Header */}
        <div className="wcb-header">
          <h3>Why Choose Brandmingo for CRM Development</h3>
          <p className="wcb-subtitle">
            We build secure, scalable, and high-performance CRM solutions that
            streamline business operations, improve customer relationships, and
            drive long-term growth.
          </p>
        </div>

        {/* Grid */}
        <div className="wcb-grid">
          {/* 1 */}
          <div className="wcb-card">
            <div className="wcb-stat-value">
              3<span>+</span>
            </div>
            <div className="wcb-stat-label">Years of CRM Expertise</div>
            <p className="wcb-stat-desc">
              Every CRM is designed around your unique business processes and
              workflows.
            </p>
          </div>

          {/* 2 */}
          <div className="wcb-card">
            <div className="wcb-feat-icon">
              <i className="fas fa-wordpress" />
            </div>
            <div className="wcb-feat-title">Expert CRM Developers</div>
            <p className="wcb-feat-desc">
              Skilled team focused on performance, usability, and scalable
              CRM solutions.
            </p>
          </div>

          {/* 3 */}
          <div className="wcb-card">
            <div className="wcb-stat-value">
              100<span>%</span>
            </div>
            <div className="wcb-stat-label">Custom Solutions</div>
            <p className="wcb-stat-desc">
              Successfully built custom CRM solutions for startups, businesses, and growing
              brands.
            </p>
          </div>

          {/* 4 */}
          <div className="wcb-card">
            <div className="wcb-stat-value">
              98<span>%</span>
            </div>
            <div className="wcb-stat-label">Client Satisfaction</div>
            <p className="wcb-stat-desc">
              Consistently delivering projects that exceed expectations and
              performance goals.
            </p>
          </div>

          {/* 5 */}
          <div className="wcb-card">
            <div className="wcb-feat-icon">
              <i className="fas fa-bolt" />
            </div>
            <div className="wcb-feat-title">Fast & Agile Delivery</div>
            <p className="wcb-feat-desc">
              Efficient workflows and quick iterations ensure faster project
              completion.
            </p>
          </div>

          {/* 6 */}
          <div className="wcb-card">
            <div className="wcb-feat-icon">
              <i className="fas fa-headset" />
            </div>
            <div className="wcb-feat-title">24×7 Support</div>
            <p className="wcb-feat-desc">
              Reliable support to keep your website secure, updated, and running
              smoothly.
            </p>
          </div>

          {/* 7 */}
          <div className="wcb-card wcb-card-highlight">
            <div className="wcb-stat-value">
              100<span>%</span>
            </div>
            <div className="wcb-stat-label">In-House Team</div>
            <p className="wcb-stat-desc">
              No outsourcing — every project handled by our dedicated in-house
              experts.
            </p>
          </div>

          {/* 8 */}
          <div className="wcb-card">
            <div className="wcb-feat-icon">
              <i className="fas fa-shield-alt" />
            </div>
            <div className="wcb-feat-title">Quality Assurance</div>
            <p className="wcb-feat-desc">
              Every website is tested for speed, security, and seamless user
              experience.
            </p>
          </div>

          {/* 9 */}
          <div className="wcb-card">
            <div className="wcb-stat-value">
              99<span>%</span>
            </div>
            <div className="wcb-stat-label">On-Time Delivery</div>
            <p className="wcb-stat-desc">
              Projects delivered on schedule without compromising quality or
              performance.
            </p>
          </div>

          {/* 10 */}
          <div className="wcb-card">
            <div className="wcb-feat-icon">
              <i className="fas fa-chart-line" />
            </div>
            <div className="wcb-feat-title">Performance & Growth Focused</div>
            <p className="wcb-feat-desc">
              Websites optimized for speed, SEO, and long-term business growth.
            </p>
          </div>

          {/* 11 */}
          <div className="wcb-card">
            <div className="wcb-stat-value">
              90<span>%</span>
            </div>
            <div className="wcb-stat-label">Client Retention</div>
            <p className="wcb-stat-desc">
              Strong relationships built through consistent results and trusted
              service.
            </p>
          </div>

          {/* 12 */}
          <div className="wcb-card">
            <div className="wcb-feat-icon">
              <i className="fas fa-layer-group" />
            </div>
            <div className="wcb-feat-title">Flexible Engagement</div>
            <p className="wcb-feat-desc">
              Custom plans tailored to your business needs, goals, and budget.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
};

export default CRMWhyChooseBm;
